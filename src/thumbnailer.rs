...
use crossbeam_channel::{unbounded, Sender as CSender, Receiver as CReceiver, TryRecvError as CTryRecvError};
use std::thread;
use std::fs;
    ...
    thread::spawn(move || {
        for task in rx.iter() {
                if let Ok(()) = generate_thumbnail(&task.src, task.size) {
                    ...
                    let _ = res_tx_clone.send((task.src.to_string_lossy().to_string(), dst.to_string_lossy().to_string()));
                }
            }
    });
...
fn is_video_ext(ext: &str) -> bool {
    matches!(ext.to_lowercase().as_str(), "mp4"|"mkv"|"webm"|"avi"|"mov"|"mpeg"|"mpg")
}
...
fn generate_thumbnail(src: &Path, size: u32) -> Result<(), String> {
    ...
    if let Some(ext) = src.extension().and_then(|e| e.to_str()) {
        if ext.eq_ignore_ascii_case("kra") {
            let tools = ["gnome-kra-thumbnailer", "gnome-openraster-thumbnailer", "kra-thumbnailer"];
            for tool in &tools {
    ...
    let try_paths = || -> Option<PathBuf> {
        if icon_name.contains('/') {
            ...
            let p = if icon_name.starts_with('/') {
                ...
                src.parent().unwrap_or_else(|| Path::new(".")).join(&icon_name)
            };
            if p.exists() {
                return Some(p);
            }
...
fn try_extract_icon_from_appimage(src: &Path, dst: &Path, size: u32) -> Result<bool, String> {
    ...
    let out = Command::new("bsdtar").arg("-tf").arg(src.as_os_str()).output();
    ...
    for line in list.lines() {
        ...
        if ll.ends_with(".png") || ll.ends_with(".svg") || ll.ends_with(".xpm") || ll.ends_with(".jpg") || ll.ends_with(".jpeg") {
            if ll.contains("/icons/") || ll.contains("/pixmaps/") || ll.contains("/apps/") || ll.contains("icon") {
                // prefer larger sizes if indicated
                if candidate.is_none() || ll.contains("128") || ll.contains("256") || ll.contains("512") {
                    candidate = Some(l.to_string());
                    if ll.contains("256") || ll.contains("512") { break; }
                }
            }
        }
    }

    let candidate = match candidate { Some(c) => c, None => return Ok(false) };
    ...
    let status = Command::new("bsdtar").arg("-xOf").arg(src.as_os_str()).arg(candidate).output();
...
fn run_icon_to_dst(icon_path: &Path, dst: &Path, size: u32) -> Result<bool, String> {
    ...
    let ext = icon_path.extension().and_then(|e| e.to_str()).unwrap_or("").to_lowercase();
    ...
    if raster_exts.contains(&ext.as_str()) {
        if let Ok(img) = image::open(icon_path) {
            ...
            let y = (size.saturating_sub(new_h)) / 2;
            let _ = canvas.copy_from(&resized.to_rgba8(), x, y);
            if let Err(e) = canvas.save(dst) { return Err(format!("save icon: {}", e)); }
            return Ok(true);
        }
    }
...
fn extract_preview_from_kra(src: &Path, dst: &Path, size: u32) -> Result<bool, String> {
    ...
    let candidates = ["preview.png", "Preview.png", "mergedimage.png", "mergedimage.jpg", "Thumbnails/thumbnail.png", "Thumbnails/thumbnail.jpg"];
    ...
    for i in 0..archive.len() {
        if let Ok(mut f) = archive.by_index(i) {
            ...
            if name.to_lowercase().ends_with(".png") || name.to_lowercase().ends_with(".jpg") || name.to_lowercase().ends_with(".jpeg") {
                let mut buf: Vec<u8> = Vec::new();
                use std::io::Read;
                if let Err(_) = f.read_to_end(&mut buf) {
                    continue;
...
fn load_system_thumbnailers() -> Option<&'static Vec<SystemThumb>> {
    ...
    let dirs = vec![
        ...
        dirs::home_dir().map(|d| d.join(".local/share/thumbnailers")).unwrap_or_default(),
    ];
                        ...
                        if ext == "thumbnailer" {
                            if let Ok(s) = fs::read_to_string(e.path()) {
                                ...
                                if let Some(exec) = exec_line {
                                    if !mimes.is_empty() {
                                        out.push(SystemThumb { mime_globs: mimes, exec });
                                    }
                                }
                            }
                        }
...
pub fn enqueue(src: &Path, size: u32) {
    let sender = init();
    let _ = sender.send(Task { src: src.to_path_buf(), size });
}
