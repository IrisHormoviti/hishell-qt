...
impl FileManagerRust {
	pub fn copy_file(&self, source: &QString, dest: &QString) -> bool {
		let src = clean_path(&source.to_string());
		let dst = clean_path(&dest.to_string());
		fs::copy(src, dst).is_ok()
	}
	pub fn duplicate_file(&self, source: &QString) -> bool {
	...
	pub fn new_folder(&self, parent: &QString) -> bool {
	...
	pub fn new_text_file(&self, parent: &QString) -> bool {
		let parent_string = clean_path(&parent.to_string());
		let parent_path = Path::new(&parent_string);
		if !parent_path.is_dir() {
			return false;
	...
	pub fn create_link(&self, source: &QString, dest: &QString) -> bool {
		let src = clean_path(&source.to_string());
	...
	pub fn trash_file(&self, path: &QString) -> bool {
		let p = clean_path(&path.to_string());
		trash::delete(p).is_ok()
	}
	pub fn delete_file(&self, path: &QString) -> bool {
		let p = clean_path(&path.to_string());
		let p = Path::new(&p);
		if p.is_dir() {
	...
	pub fn rename_file(&self, path: &QString, new_name: &QString) -> bool {
		let p = clean_path(&path.to_string());
	...
	pub fn copy_paths_to_clipboard(&self, paths_newline: &QString) -> bool {
		...
		if !uris.is_empty() {
			if let Ok(mut guard) = INTERNAL_CLIPBOARD.lock() {
				*guard = Some((uris.clone(), false));
			}
		}
		set_clipboard_uris(&paths, false)
	}
	pub fn cut_paths_to_clipboard(&self, paths_newline: &QString) -> bool {
		let paths = paths_newline.to_string();
		let uris: Vec<String> = paths
			.lines()
			.filter(|l| !l.trim().is_empty())
		...
		if !uris.is_empty() {
			if let Ok(mut guard) = INTERNAL_CLIPBOARD.lock() {
				*guard = Some((uris.clone(), true));
			}
		}
		set_clipboard_uris(&paths, true)
	}
	pub fn paste_from_clipboard(&self, dest_dir: &QString) -> bool {
	...
	pub fn get_mime_type(&self, path: &QString) -> QString {
		let p = clean_path(&path.to_string());
	...
	pub fn get_text_content(&self, path: &QString) -> QString {
		let p = clean_path(&path.to_string());
	...
	pub fn open_file(&self, path: &QString) -> bool {
		let p = clean_path(&path.to_string());
		crate::directory::open_file(Path::new(&p));
		true
	}

	pub fn get_icon(&self, path: &QString) -> QString {
		let p = clean_path(&path.to_string());
		let icon_str = crate::directory::get_icon(Path::new(&p));
		QString::from(icon_str.as_str())
	}

	pub fn open_file_with_dialog(&self, path: &QString) -> bool {
		let p = clean_path(&path.to_string());
		crate::portal::open_file_with_portal(Path::new(&p))
	}

	pub fn rotate_image(&self, path: &QString, degrees: i32) -> bool {
		let p = clean_path(&path.to_string());
		crate::image_utils::rotate_image(Path::new(&p), degrees)
	}

	pub fn is_image_file(&self, path: &QString) -> bool {
		let p = clean_path(&path.to_string());
		crate::image_utils::is_image_file(Path::new(&p))
	}

	pub fn is_directory(&self, path: &QString) -> bool {
		let p = clean_path(&path.to_string());
		Path::new(&p).is_dir()
	}
}
